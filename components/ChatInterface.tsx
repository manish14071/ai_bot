  ...
  return (
    <div className="flex flex-col h-[80vh]">
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`flex items-start space-x-2 ${m.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
              <Avatar>
                <AvatarFallback>{m.role === "user" ? user?.firstName?.charAt(0) || "U" : "AI"}</AvatarFallback>
              </Avatar>
        ...
        {imageUrl && (
          <div className="flex justify-center">
            <img src={imageUrl || "/placeholder.svg"} alt="Uploaded" className="max-w-xs rounded-lg shadow-md" />
          </div>
        )}
      </div>
      <form onSubmit={handleSubmit} className="p-4 space-y-4">
        ...
        <CldUploadButton uploadPreset="YOUR_UPLOAD_PRESET" onSuccess={handleUploadSuccess}>
